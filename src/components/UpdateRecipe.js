...
function UpdateRecipe() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await Axios.put(`https://recipe-finder-backend5.onrender.com/api/recipes/update-recipe/${recipeId}`, recipe);
  ...
  return (
    <div>
      <h1 className='sub_heading text-center'>Update the Recipe</h1>
      <form onSubmit={handleSubmit} className='w-50 sub_form' enctype="multipart/form-data" method="POST">
        ...
        <button type="button" className='btn btn-primary fw-bold' onClick={addIngredientField}>Add Ingredient</button>
          ...
          <div className='col-12 pt-5 text-center pb-5'>
            <button className='btn btn-success fw-bold' type='submit'>Update Recipe</button>
          </div>
        </div>
      </form>
    </div>
