...
function Home() {
  ...
  useEffect(() => {
    async function fetchLatestRecipes() {
      try {
        const response = await Axios.get('http://localhost:4000/api/recipes/latest-recipes');
          ...
          <div key={category._id} class="category_link">
            <Link to={`/recipes-by-category/${category.name}`} className='link_text'>
              <div class="category_img">
                <img src={`http://localhost:4000/images/${category.image}`} alt={category.name} width="250" height="250"/> <br/>
            ...
            <div key={recipe._id} className="category_link">
              <Link to={`/recipes/${recipe._id}`} className="link_text">
                <div className="category_img">
                  <img src={`http://localhost:4000/images/${recipe.image}`} alt={recipe.name} width="250" height="250" /> <br />
